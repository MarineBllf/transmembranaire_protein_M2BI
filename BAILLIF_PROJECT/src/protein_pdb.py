...
def PDB_manipulation(pdb_name): 
    ...
    dssp = DSSP(structure[0],pdb_name)
...
def build_memb(planes_eq,pdb_file,outpout_name): 
    ...
    for x in np.arange(xmin-10 , xmax+10) :
        for y in np.arange(ymin-10  , ymax+10):
    ...
    with open(pdb_file,"r") as open_file: 
        aa_lines = open_file.readlines()
